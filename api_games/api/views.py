...
def get_statistic(request):
    stats = Statistic.objects.all().values('id', 'player_id', 'score', 'time_played', 'game_id')
...
def top_players(request):
    top_players = Statistic.objects.values('player__name').annotate(total_time_played=Sum('time_played')).order_by('-total_time_played')[:5]
